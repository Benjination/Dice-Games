...
/// Service for managing Pig Dice leaderboard scores
class PigService {
  static final _firestore = FirebaseFirestore.instance;

  /// Submit a score to the Pig Dice leaderboard
  static Future<void> submitScore({
    required String userId,
    required String username,
    required int score,
  }) async {
    final scoreEntry = PigScore(
      username: username,
      score: score,
  ...
  static Future<List<PigScore>> getTopScores({int limit = 15}) async {
    ...
    return snapshot.docs
        .map((doc) => PigScore.fromJson(doc.data()))
        .toList();
  }

  /// Get user's personal best score
  static Future<PigScore?> getUserBestScore(String userId) async {
    final snapshot = await _firestore
        .collection('pig_leaderboard')
    ...
    return PigScore.fromJson(snapshot.docs.first.data());
  }
